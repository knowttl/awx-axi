...
- Current v1 command coverage is all twelve registered `DOMAINS` in `src/cli.ts`: `job`, `template`, `workflow`, `organization`, `credential`, `approval`, `ad-hoc`, `project`, `inventory`, `schedule`, `execution-environment`, `user`.
...
- Remaining roadmap priorities are: teams and RBAC context, notifications and activity stream, and system job coverage.
