...
- A subcommand declares its positional arity as `maxArgs`, and a domain result is either a `Renderable` or `withExitCode(renderable, code)` - the one channel for design §7.9's "exit 1 but still print the job block".
