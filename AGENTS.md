...
- Current v1 command coverage is all registered `DOMAINS` in `src/cli.ts`: 12 domains total, `job`, `template`, `workflow`, `approval`, `ad-hoc`, `project`, `inventory`, `schedule`, `execution-environment`, `organization`, `credential`, and `user`.
