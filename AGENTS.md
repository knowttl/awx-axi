...
- Current shipped coverage is all registered `DOMAINS` in `src/cli.ts`: `job`, `template`, `workflow`, `ad-hoc`, `approval`, `project`, `inventory`, `schedule`, `execution-environment`, `organization`, `credential`, and `user` (12).
...
- Remaining roadmap priorities are: teams and RBAC context, notifications and activity stream, and system jobs.
