...
- Current shipped coverage is all twelve registered `DOMAINS` in `src/cli.ts`: `job`, `template`, `workflow`, `organization`, `credential`, `approval`, `ad-hoc`, `project`, `inventory`, `schedule`, `execution-environment`, and `user`.
...
- Remaining roadmap priorities are: ad hoc command launch/diagnostic depth, teams and RBAC context, notifications and activity stream, and system job coverage.
