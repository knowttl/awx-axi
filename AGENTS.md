...
- **All v1 domains are implemented** under `src/domains/` (job, template, workflow, approval, project, inventory, schedule, execution-environment, organization, credential, user) and registered in `DOMAINS` in `src/cli.ts`. The approval domain (`src/domains/approval/index.ts`) is the reference for `defineDomain` shape, a resolve-then-read plan (`approval show <id|name>` via core `resolveId`), and a derived field built from a second read (`blocks` from the workflow's node list).
...
- Current v1 command coverage is the eleven domains in `src/domains/` and their
`DOMAINS` registration in `src/cli.ts`, including inventories and related resources.
...
- Prioritize the API roadmap as schedules, ad hoc command surface depth, organizations/teams/users and RBAC context, execution environments, notifications and activity stream, system jobs, and finally credentials.
