...
- **The first concrete domain is `src/domains/approval/index.ts`** (read-only inbox, design §7.7): the reference for `defineDomain` shape, a resolve-then-read plan (`approval show <id|name>` via core `resolveId`), and a derived field built from a second read (`blocks` from the workflow's node list). Registered in `DOMAINS` in `src/cli.ts`.
